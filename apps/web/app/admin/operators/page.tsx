            ...
            <div className="container px-4 md:px-6">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
                    <div className="space-y-4">
                        <Link href="/admin" className="inline-flex items-center gap-2 text-gray-500 hover:text-[#FB7427] transition-colors text-[10px] font-black uppercase tracking-widest mb-4">
                            <ArrowLeft className="w-4 h-4" /> Admin
                        </Link>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#5BC2E7]/10 border border-[#5BC2E7]/20">
                            <UserPlus className="w-3.5 h-3.5 text-[#5BC2E7]" />
                            <span className="text-[10px] font-bold text-[#5BC2E7] uppercase tracking-[0.2em]">Approval queue</span>
                        </div>
                        <h1 className="text-4xl md:text-6xl font-black text-white uppercase italic tracking-tighter">
                            Operator <span className="text-[#5BC2E7]">verification</span>
                        </h1>
                ...
                <div className="space-y-6">
                    {pendingOperators.length === 0 ? (
                        ...
                        pendingOperators.map((operator: any) => (
                            <div key={operator.id} className="p-8 rounded-2xl bg-[#18222e] border border-white/5 hover:border-[#5BC2E7]/30 transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 group">
                                <div className="space-y-4 flex-1">
                                    ...
                                    <h3 className="text-2xl font-black text-white group-hover:text-[#5BC2E7] transition-colors uppercase italic tracking-tighter">
                                        {operator.name}
                                    </h3>

                                    <div className="flex flex-wrap gap-4 text-[10px] font-bold text-gray-500 tracking-widest uppercase">
                                        <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-[#5BC2E7]" /> {operator.radius ? `${operator.radius} km range` : "Service area on request"}</span>
                                        <span className="flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-[#5BC2E7]" /> {operator.services.split(',').length} services</span>
                                    </div>
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {operator.documents.map((doc: any) => (
                                                    <a key={doc.id} href={doc.url} target="_blank" rel="noopener noreferrer"
                                                        className="flex items-center gap-2 text-xs font-bold text-gray-300 bg-black/40 border border-white/5 hover:border-[#5BC2E7]/40 px-3 py-1.5 rounded-lg transition-all">
                                ...
                                <div className="flex items-center gap-3 w-full md:w-auto">
                                    ...
                                    <form action={async () => {
                                        ...
                                        <Button className="h-14 px-8 rounded-xl bg-[#5BC2E7] hover:bg-[#5BC2E7] text-black font-black uppercase tracking-tighter transition-all flex items-center gap-2 group-hover:scale-105">
